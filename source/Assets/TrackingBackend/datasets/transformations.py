...
class ConvertKeypoints:
    def __call__(self, sample):
        ...
        for (
            keypoint
        ) in keypoints:  # keypoint[2] == 0: occluded, == 1: visible, == 2: not in image
...
class Rotate:
    ...
    def __call__(self, sample):
        ...
        sample["label"]["img_height"], sample["label"]["img_width"], _ = sample[
            "image"
        ].shape
    ...
    def _rotate(self, point, R):
        return [
            R[0, 0] * point[0] + R[0, 1] * point[1] + R[0, 2],
...
class CropPad:
    ...
    def __call__(self, sample):
        ...
        if should_crop:
            cropped_image[
                crop_y_start:crop_y_finish, crop_x_start:crop_x_finish, :
            ] = sample["image"][
                image_y_start:image_y_finish, image_x_start:image_x_finish, :
            ]
            cropped_mask[
                crop_y_start:crop_y_finish, crop_x_start:crop_x_finish
            ] = sample["mask"][
...
class Flip:
    ...
    def __call__(self, sample):
        ...
        for other_annotation in label["processed_other_annotations"]:
            ...
            for keypoint in other_annotation["keypoints"]:
                keypoint[0] = w - 1 - keypoint[0]
            other_annotation["keypoints"] = self._swap_left_right(
