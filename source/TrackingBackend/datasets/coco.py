...
class CocoTrainDataset(Dataset):
    def __init__(
        self, labels, images_folder, stride, sigma, paf_thickness, transform=None
    ):
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(
            os.path.join(self._images_folder, label["img_paths"]), cv2.IMREAD_COLOR
        )
        mask = np.ones(
            shape=(label["img_height"], label["img_width"]), dtype=np.float32
        )
        mask = get_mask(label["segmentations"], mask)
        sample = {"label": label, "image": image, "mask": mask}
        if self._transform:
    ...
    def _generate_keypoint_maps(self, sample):
        ...
        keypoint_maps = np.zeros(
            shape=(n_keypoints + 1, n_rows // self._stride, n_cols // self._stride),
            dtype=np.float32,
        )  # +1 for bg
    ...
    def _generate_paf_maps(self, sample):
        ...
        n_rows, n_cols, _ = sample["image"].shape
        paf_maps = np.zeros(
            shape=(n_pafs * 2, n_rows // self._stride, n_cols // self._stride),
            dtype=np.float32,
        )
        ...
        for paf_idx in range(n_pafs):
            ...
            if keypoint_a[2] <= 1 and keypoint_b[2] <= 1:
                self._set_paf(
                    paf_maps[paf_idx * 2 : paf_idx * 2 + 2],
                    keypoint_a[0],
                    keypoint_a[1],
                    keypoint_b[0],
                    keypoint_b[1],
            ...
            for another_annotation in label["processed_other_annotations"]:
                keypoint_a = another_annotation["keypoints"][
                    BODY_PARTS_KPT_IDS[paf_idx][0]
                ]
                keypoint_b = another_annotation["keypoints"][
                    BODY_PARTS_KPT_IDS[paf_idx][1]
                ]
                if keypoint_a[2] <= 1 and keypoint_b[2] <= 1:
                    self._set_paf(
                        paf_maps[paf_idx * 2 : paf_idx * 2 + 2],
